...
from pages.backend.lv.LVDefinitions import *
...
selected_lv_name = col_lvSelection_1.selectbox("Choose an LV supply", lvSupplyNameList)

# check if the selected LV is different from the current one 
if selected_lv_name != st.session_state.lv.name:
	st.session_state.lvid = getLVID(selected_lv_name)
	st.session_state.lv = lvSupplyList[st.session_state.lvid]
	st.rerun()
...
if detectedMac != st.session_state.lv.manuallyEnteredMAC:
	st.warning(":warning: The detected MAC (\"" + detectedMac + "\") \
		...
		Please check carefully and correct LVDefinitions.py afterwards.")
	
if st.session_state.lv.isInConstantCurrentMode():
	st.warning(":warning: This LV supply is in constant-current mode:\
		It will increase its output voltage until the target current is reached!")
